...
# позволяет получить аргументы из командной строки при вызове скрипта
sys.argv[1:]
...
# Перегрузка операторов
'''
	Перегрузка операторов — один из способов реализации полиморфизма, когда мы можем создать в классе наследнике метод с таким же название, что и в классе родителе, но работать они будут по разному.
'''
class A:
    def go(self):
        print('Go, A!')
...
'''
# конструктор класса, который срабатывает при создании экземпляра класса (объекта)
__init__(self[, ...])
...
# вызывается, когда атрибут экземпляра класса не найден
...
# Модули/Пакеты
'''
	__init__.py - позволяет питону интерпритировать директорию, как модуль (кладётся в директорию, где храниться модуль)

	__init__.py позволяет загружать модуль с помощью конструкции from/import

	from __future__ import absolute_import, unicode_literals

	Файл, который открыли первым считается главным файлом. Его имя будет __main__

	Когда мы выполняем файл через консоль внутри него могут подключаться другие файлы с кодом. При чтении этих файлов могут возникать ошибки из-за того, что интерпретатор будет просто считывать код, не заботясь о передаче аргументов в функции. 

	Чтобы избежать этого мы должны обернуть весь код файла например в функцию main(), а затем проверить является ли этот файл главным. Если да, то выполняем фукнцию main(), если нет, то ничего не делаем.
