...
SERVER_EMAIL = 'magic_local@error.com'
EMAIL_DEFAULT_FROM = 'magic_local@error.com'

# Отправляет сообщение об успешном выполнении задачи в rabbit только по её
# успешному выполнению
# По умолчанию (False) отправляет сообщение об успешном выполнении задачи до
# её выполнения
...
# Путь и настройки к хранилищу на CDN (использовать только если нужно
# протестировать в боевом режиме)
...
# Если нет доступа к cdn по ключу, то
SFTP_CONNECTIONS = {
    'cdn': {
        'host': '10.10.100.5',
        'port': 22,
...
# авторизация на cdn по ключу, если будет другой сервер - не забыть про
# авторизацию
...
# запускается несколько задач параллельно по COUNT_ITERATED элементов
# [[1, 2], [3, 4], ...]
COUNT_ITERATED = 10
